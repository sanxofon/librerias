...
var PDFView = {
  ...
  initialize: function pdfViewInitialize() {
    ...
    setTimeout('PDFView.setHash(document.location.hash.substring(1))',1000);
  },
  ...
  parseScale: function pdfViewParseScale(value, resetAutoSettings) {
    ...
    var pageWidthScale = ((container.clientWidth/2) - kScrollbarPadding) /
    ...
    switch (value) {
      case 'page-actual':
        this.setScale(1, true); //resetAutoSettings);
        break;
      case 'page-width':
        this.setScale(pageWidthScale, true); //resetAutoSettings);
        break;
      case 'page-height':
        this.setScale(pageHeightScale, true);  //resetAutoSettings);
        break;
      case 'page-fit':
        this.setScale(Math.min(pageWidthScale, pageHeightScale), true); //resetAutoSettings);
        break;
      case 'auto': //XXX
        this.setScale(Math.min(1.0, pageWidthScale), true); //resetAutoSettings);
        break;
    }
  ...
  defaultview: function pdfViewDefault() {
    var url = this.url.split('#')[0];
    window.location = '/islandora_74/sites/all/libraries/pdfjs/web/viewer.html?file='+url+'#page='+currentPageNumber;
  },
...
//Function for adding and removing class of element
function elemHasClass(ele,cls) {
    return ele.className.match(new RegExp('(\\s|^)'+cls+'(\\s|$)'));
}
function elemAddClass(ele,cls) {
    if (!elemHasClass(ele,cls)) ele.className += " "+cls;
}
function elemRemoveClass(ele,cls) {
    if (elemHasClass(ele,cls)) {
        var reg = new RegExp('(\\s|^)'+cls+'(\\s|$)');
        ele.className=ele.className.replace(reg,' ');
    }
}
function turnPageNum(num) {
  var estaPage = PDFView.page;
  var totalPages = PDFView.pages.length;
  if (num>0) {
    if (estaPage+num<=totalPages) {
      PDFView.page+=num;
    } else {
      PDFView.page++;
    }
...
window.addEventListener('load', function webViewerLoad(evt) {
  ...
  document.getElementById('viewFullScreen').addEventListener('click',
    function() {
      var i = document.getElementById("outerContainer");
      if (elemHasClass(document.getElementById('viewFullScreen'),'fullScreenOn')) {
        elemRemoveClass(document.getElementById('viewFullScreen'),'fullScreenOn');
        // exit fullscreen
        ...
        elemAddClass(document.getElementById('viewFullScreen'),'fullScreenOn');
        // go fullscreen
